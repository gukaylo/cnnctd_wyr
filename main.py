...
def button(update: Update, context: CallbackContext):
    ...
    # Если таймер запущен или нужно его запустить, показываем предупреждение
    if votes_len >= 2:
        ...
        # Показываем предупреждение о 20 секундах
        text_with_timer = "⚠️ Осталось 20 секунд!\n\n" + summary_text()
        update_message(context, text_with_timer)
    else:
        # Если меньше 2 голосов, просто обновляем список проголосовавших
        text = summary_text()
...
def main():
    ...
    if not token:
        logger.error("BOT_TOKEN не найден в переменных окружения! Создайте файл .env с BOT_TOKEN=...")
        return
